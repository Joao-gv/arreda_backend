    ...
    public String validarToken(String token) {
        try {
            ...
            return JWT.require(algorithm)
                    .withIssuer("arreda_backend") // Garante que foi nossa API que emitiu
                    .build()
                    .verify(token)          // Decodifica e valida a expiração automaticamente
                    .getSubject();          // Extrai o e-mail do usuário

        } catch (JWTVerificationException exception) {
            // Se o token estiver expirado, corrompido ou for falso, cai aqui
            return "";
        }
    }
